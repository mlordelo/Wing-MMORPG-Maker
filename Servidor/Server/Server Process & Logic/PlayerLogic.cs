    ...
    class PlayerLogic
    {
        ...
        public static void ExecuteSkill(int index)
        {
            ...
            if (PStruct.tempplayer[index].targettype == 1)
            {
                ...
                if ((DistanceX <= n) && (DistanceY <= n))
                {
                    ...
                    if (!(PStruct.character[target, PStruct.player[target].SelectedChar].Map == Map) || !(target != index))
                    {
                        ...
                        PStruct.tempplayer[index].movespeed = Globals.NormalMoveSpeed;
                        SendData.Send_MoveSpeed(1, index);
                        SendData.Send_MsgToPlayer(index, "Não foi possível executar a magia.", Globals.ColorRed, Globals.Msg_Type_Server);
                        return;
                    }
                        ...
                        if (SStruct.skill[preparingskill].repeats <= 1)
                        {
                            ...
                            if ((SStruct.skill[preparingskill].type) == 11)
                            {
                                ...
                                if (PStruct.tempplayer[target].preparingskill > 0)
                                {
                                    ...
                                    //MSG
                                    SendData.Send_ActionMsg(target, "Conjuração quebrada!", Globals.ColorPink, PStruct.character[target, PStruct.player[target].SelectedChar].X, PStruct.character[target, PStruct.player[target].SelectedChar].Y, 1, 0, Map);
                            ...
                            if ((SStruct.skill[preparingskill].type) == 4)
                            {
                                ...
                                for (int i = 0; i <= MStruct.tempmap[Map].NpcCount; i++)
                                {
                                    ...
                                    if ((DistanceX <= n) && (DistanceY <= n))
                                    {
                                        ...
                                        SendData.Send_ActionMsg(i, "Congelado", Globals.ColorWhite, NStruct.tempnpc[Map, i].X, NStruct.tempnpc[Map, i].Y, Globals.Action_Msg_Scroll, 0, Map);
                                    }
                                }
                                for (int i = 0; i <= Globals.Player_HighIndex; i++)
                                {
                                    ...
                                    if ((PStruct.character[i, PStruct.player[i].SelectedChar].Map == Map) && (i != index) && (i != target))
                                    {
                                        ...
                                        if ((DistanceX <= n) && (DistanceY <= n))
                                        {
                                            ...
                                            SendData.Send_ActionMsg(i, "Congelado", Globals.ColorWhite, PStruct.character[i, PStruct.player[i].SelectedChar].X, PStruct.character[i, PStruct.player[i].SelectedChar].Y, Globals.Action_Msg_Scroll, 0, Map);
                                        }
                                    }
                                }
                                ...
                                SendData.Send_MoveSpeed(Globals.Target_Player, target);
                                SendData.Send_ActionMsg(target, "Congelado", Globals.ColorWhite, PStruct.character[target, PStruct.player[target].SelectedChar].X, PStruct.character[target, PStruct.player[target].SelectedChar].Y, Globals.Action_Msg_Scroll, 0, Map);
            ...
            if (PStruct.tempplayer[index].targettype == 2)
            {
                    ...
                    if (SStruct.skill[preparingskill].damage_type == 1)
                    {
                        if (SStruct.skill[preparingskill].repeats <= 1)
                        {
                            ...
                            if ((SStruct.skill[preparingskill].type) == 4)
                            {
                                ...
                                SendData.Send_ActionMsg(target, "Congelado", Globals.ColorWhite, NStruct.tempnpc[Map, target].X, NStruct.tempnpc[Map, target].Y, Globals.Action_Msg_Scroll, 0, Map);
                ...
                {
                    //Sem alcance
                    SendData.Send_MsgToPlayer(index, "Você está distante demais do alvo para acerta-lo", Globals.ColorRed, Globals.Msg_Type_Server);
