                ...
                if (PStruct.character[target, PStruct.player[target].SelectedChar].ClassId == 6)
                {
                    for (int i = 1; i < Globals.MaxPlayer_Skills; i++)
                    {
                        if ((PStruct.skill[target, i].num == 39) && (PStruct.skill[target, i].level > 0))
                        {
                            ...
                            {
                                SendData.Send_ActionMsg(target, "Errou", Globals.ColorWhite, PStruct.character[target, PStruct.player[target].SelectedChar].X, PStruct.character[target, PStruct.player[target].SelectedChar].Y, 1, 0, map);
                                return;
                            }
                            break;
                        }
